<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="flex items-center justify-between">
        <div>
            <flux:heading size="xl">{{ __('Laporan Omzet Harian') }}</flux:heading>
            <flux:text class="mt-1 text-zinc-600 dark:text-zinc-400">
                {{ __('Laporan posting omzet harian perusahaan, total BV, total komisi keluar (payout), dan persentase rasio payout.') }}
            </flux:text>
        </div>
    </div>
            ...
            <table class="w-full text-left text-sm border-collapse">
                <thead>
                    <tr class="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 font-medium bg-zinc-50 dark:bg-zinc-800/20">
                        ...
                        <th class="py-3 px-4">Tanggal Omzet</th>
                    ...
                    @forelse($orders as $index => $row)
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/10">
                            ...
                            <td class="py-3 px-4 font-medium text-zinc-900 dark:text-zinc-100">
                                {{ \Carbon\Carbon::parse($row->date)->locale('id')->isoFormat('DD MMM YYYY') }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-8 text-center text-zinc-500">
                                {{ __('Tidak ada data omzet harian ditemukan.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
